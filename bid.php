...
try {
    ...
    if ($user['bid_coin_balance'] < 10) {
        throw new Exception('Not enough bids');
    }

    // Update user bids
    $query = mysqli_prepare($conn, 'UPDATE users SET bid_coin_balance = bid_coin_balance - 10 WHERE id = ?');
    mysqli_stmt_bind_param($query, "i", $userId);
    ...
    $_SESSION['bid_coin_balance'] = $user['bid_coin_balance'] - 10;

    // Update auction
    $query = mysqli_prepare($conn, 'UPDATE auctions SET current_bid = current_bid + 10 WHERE id = ?');
